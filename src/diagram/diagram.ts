...
export class Diagram extends Component<HTMLElement> implements INotifyPropertyChanged {
            ...
            for (let node of Object.keys((layer as Layer).zIndexTable)) {
                ...
                if (!(this.nameTable[(layer as Layer).zIndexTable[node]].parentId)
                    ...
                    renderer.renderElement(
                        ...
                        (!renderer.isSvgMode && transform) ? this.scroller.transform : undefined);
                }
            }
        }
    }
    ...
    public clearCanvas(view: View): void {
        let width: number;
        let height: number;
        width = view.contentWidth || (view.diagramLayer as HTMLCanvasElement).width;
        height = view.contentHeight || (view.diagramLayer as HTMLCanvasElement).height;
        if (view.mode !== 'SVG') {
    ...
    public connectorPropertyChange(actualObject: Connector, oldProp: Connector, newProp: Connector, disableBridging?: boolean): void {
        ...
        if (newProp.visible !== undefined) { this.updateElementVisibility(actualObject.wrapper, newProp, actualObject.visible); }
