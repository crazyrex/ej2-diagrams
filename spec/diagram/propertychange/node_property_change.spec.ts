...
import { TextElement, TextStyleModel, PathElement, NodeConstraints } from '../../../src/diagram/index';
...
describe('Diagram Control', () => {
    ...
    describe('Node  property change', () => {
        ...
        beforeAll((): void => {
            ...
            let node1: NodeModel = {
                id: 'node', width: 100, height: 100, offsetX: 500, offsetY: 300, backgroundColor: 'red',
                shape: {
                    type: 'Path', data:
                        'M540.3643,137.9336L546.7973,159.7016L570.3633,159.7296L550.7723,171.9366L558.9053,194.9966' +
                        'L540.3643,179.4996L521.8223,194.9966' +
        ...
        it('Checking node width property change', (done: Function) => {
            diagram.nodes[1].width = 300;
            diagram.dataBind();
        ...
        it('Checking node height property change', (done: Function) => {
            diagram.nodes[1].height = 300;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[1];
            expect(node.height == 300).toBe(true);
            done();
        });

        it('Checking node offsetX property change', (done: Function) => {
            diagram.nodes[0].offsetX = 200;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.offsetX == 200).toBe(true);
            done();
        });

        it('Checking node offsetY property change', (done: Function) => {
            diagram.nodes[0].offsetY = 70;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.offsetY == 70).toBe(true);
            done();
        });
        ...
        it('Checking node strokeWidth property change', (done: Function) => {
            diagram.nodes[1].style.strokeDashArray = '5';
            diagram.nodes[1].style.strokeWidth = 5;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[1];
            expect(node.style.strokeWidth == 5 && node.style.strokeDashArray == '5').toBe(true);
            done();
        });
        it('Checking node backgroundcolor change', (done: Function) => {
            diagram.nodes[0].backgroundColor = 'grey'
            diagram.dataBind();
            let element: HTMLElement = document.getElementById(diagram.nodes[0].id);
            expect(element.attributes[14].value === 'Grey')
            done();
        });

        it('Checking node pivot property change', (done: Function) => {
            diagram.nodes[0].pivot = { x: 1, y: 1 };
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.pivot.x == 1 && node.pivot.y == 1).toBe(true);
            done();
        });
        it('Checking node rotateAngle property change', (done: Function) => {
            diagram.nodes[0].rotateAngle = 90;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.rotateAngle == 90).toBe(true);
            done();
        });
        it('Checking node minHeight property change', (done: Function) => {
            (diagram.nodes[0] as NodeModel).minHeight = 155;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.minHeight == 155).toBe(true);
            done();
        });
        it('Checking node minWidth property change', (done: Function) => {
            (diagram.nodes[0] as NodeModel).minWidth = 190;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.minWidth == 190).toBe(true);
            done();
        });
        it('Checking node maxWidth property change', (done: Function) => {
            (diagram.nodes[0] as NodeModel).maxWidth = 80;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.maxWidth == 80).toBe(true);
            done();
        });
        it('Checking node maxHeight property change', (done: Function) => {
            (diagram.nodes[0] as NodeModel).maxHeight = 70;
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect(node.maxHeight == 70).toBe(true);
            done();
        });

        it('Checking text content change', (done: Function) => {
            ((diagram.nodes[1] as NodeModel).shape as TextModel).content = 'NewContent';
            ((diagram.nodes[1] as NodeModel).style as TextStyleModel).bold = true;
            ((diagram.nodes[1] as NodeModel).style as TextStyleModel).textWrapping = 'NoWrap';
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[1];
            expect((node.wrapper.children[0] as TextElement).content).toBe('NewContent');
            done();
        });

        it('Checking path node data property change', (done: Function) => {
            ((diagram.nodes[0] as NodeModel).shape as PathModel).data = 'M100,100 L140,100 L 120,140z';
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[0];
            expect((node.shape as PathModel).data == 'M100,100 L140,100 L 120,140z').toBe(true);
            done();
        });

        it('Checking flow shape change ', (done: Function) => {
            let pathNode: NodeModel = {
                id: 'node', width: 100, height: 100, offsetX: 500, offsetY: 100,
                style: { fill: 'green' },
                shape: {
            ...
            ((diagram.nodes[2] as NodeModel).shape as FlowShapeModel).shape = 'OffPageReference';
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[2];
            expect((node.shape as FlowShapeModel).shape == 'OffPageReference').toBe(true);
            done();
        });

        it('Checking basic shape change ', (done: Function) => {
            let pathNode: NodeModel = {
                id: 'node', width: 100, height: 100, offsetX: 500, offsetY: 100,
                style: { fill: 'green' },
                shape: {
            ...
            diagram.add(pathNode);
            ((diagram.nodes[3] as NodeModel).shape as BasicShapeModel).shape = 'Diamond';
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[3];
            expect((node.shape as BasicShapeModel).shape == 'Diamond').toBe(true);
            done();
        });

        it('Checking image source change ', (done: Function) => {
            let shape: ImageModel = { type: 'Image', source: 'https://www.w3schools.com/images/w3schools_green.jpg' };
            let imageNode: NodeModel = {
                id: 'image', width: 100, height: 100, offsetX: 300, offsetY: 300,
                shape: shape
            };
            diagram.add(imageNode);
            ((diagram.nodes[4] as NodeModel).shape as ImageModel).source = '';
            diagram.dataBind();
            let node: NodeModel = diagram.nodes[4];
            expect((node.shape as ImageModel).source == '').toBe(true);
            done();
        });
        it('Changing image shape at run time', (done: Function) => {
            diagram.nodes[2].shape = {
            ...
            expect(value.childNodes[1].attributes[8].nodeValue === 'https://www.w3schools.com/images/w3schools_green.jpg').toBe(true);
            done();
        });
        it('Changing path shape at run time', (done: Function) => {
            diagram.nodes[2].shape = { type: 'Path', data: 'M540.3643,137.9336L546.7973,159.7016L570.3633,159.7296L550.7723,171.9366L558.9053,194.9966L540.3643,179.4996L521.8223,194.9966L529.9553,171.9366L510.3633,159.7296L533.9313,159.7016L540.3643,137.9336z' };
            diagram.dataBind();
            let value: HTMLElement = document.getElementById('node1_content_groupElement');
            expect(value.childNodes[0].attributes[0].nodeValue === "M50,0 L60.72,38.15 L100,38.2 L67.35,59.59 L80.9,100 L50,72.84 L19.1,100 L32.65,59.59 L0,38.2 L39.28,38.15 L50,0 Z ").toBe(true);
            done();
        });
        it('Changing Basic shape at run time', (done: Function) => {
        ...
        it('Changing Flow shape at run time', (done: Function) => {
            diagram.nodes[2].shape = { type: 'Flow', shape: 'SequentialData' }
            diagram.dataBind();
            let value: HTMLElement = document.getElementById('node1_content_groupElement');
            expect(value.childNodes[0].attributes[0].nodeValue === "M0,50 C0,22.38,20.86,0,46.59,0 C72.31,0,93.17,22.38,93.17,50 C93.17,77.61,72.31,100,46.59,100 C20.86,100,0,77.61,0,50 Z M46.58,100 L100,100 ").toBe(true);
            done();
        });
        it('Changing Text shape at run time', (done: Function) => {
            diagram.nodes[2].shape = { type: 'Text', content: 'Text Element' }
            diagram.dataBind();
            let value: HTMLElement = document.getElementById('node1_content_groupElement');
            done();
        });
        it('Changing Native shape at run time', (done: Function) => {
            ...
            diagram.dataBind();
            let value: HTMLElement = document.getElementById('node1_content_native_element');
            expect(value != null).toBe(true);
            done();
        });
        it('Checking image to basic shape', (done: Function) => {
            let node: NodeModel = diagram.nodes[4];
            // node.shape.type = 'Basic';
            //  diagram.dataBind();
            //  expect((node.wrapper.children[0] as PathElement).absolutePath == 'M 0 0 L 100 0 L 100 100 L 0 100 Z').toBe(true);
            done();
        });

        it('Checking basic to flow shape', (done: Function) => {
            let node: NodeModel = diagram.nodes[4];
            //node.shape.type = 'Flow';
            // diagram.dataBind();
            //  expect((node.wrapper.children[0] as PathElement).absolutePath == 'M 0 0 L 100 0 L 100 100 L 0 100 Z').toBe(true);
            done();
        });

        it('Changing shadow constraints at runtime', (done: Function) => {
            let node: NodeModel = diagram.nodes[4];
            node.constraints |= NodeConstraints.Shadow;
            diagram.dataBind();
            expect(node.wrapper.children[0].shadow != null).toBe(true);
            node.constraints &= ~NodeConstraints.Shadow;
            diagram.dataBind();
            expect(node.wrapper.children[0].shadow == null).toBe(true);
